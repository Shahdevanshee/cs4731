...
def myCreatePathNetwork(world, agent = None):
    ...
    for p1 in polys:
        ...
        # Optimized edge placement
        for i in range(len(temp)):
            if i == len(temp) - 1:
                if checkCollision3(obstacles, temp[i], temp[0], world.getAgent()):
                    edges.append((temp[i], temp[0]))
            else:
                if checkCollision3(obstacles, temp[i], temp[i + 1], world.getAgent()):
                    edges.append((temp[i], temp[i + 1]))

    # edges = myBuildPathNetwork(nodes, world, world.getAgent())
