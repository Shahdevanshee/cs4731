...
class ManualObstacle(Obstacle):
	...
	def __init__(self, points, color = (0, 0, 0), linewidth = 4, sprite = None):
		...
		self.minpt = ( min(map(lambda p: p[0], points)), min(map(lambda p: p[1], points)) )
		self.maxpt = ( max(map(lambda p: p[0], points)), max(map(lambda p: p[1], points)) )
		# create surface
		s = pygame.Surface((self.maxpt[0]+linewidth, self.maxpt[1]+linewidth), pygame.SRCALPHA, 32)
		s = s.convert_alpha()
		pygame.draw.lines(s, color, True, points, linewidth)
		self.surface = s
		self.rect = s.get_rect()
...
#		background = pygame.Surface(screen.get_size())
		...
		# unobstructed places
		self.destinations = {}
		self.halfscreendimensions = tuple(x / 2 for x in screendimensions)
...
#			self.sprites.add(n)
	...
	def drawWorld(self):
		...
		cameraMin = (-offsetX, -offsetY)
		cameraMax = tuple(cameraMin[x] + self.halfscreendimensions[x] * 2 for x in range(0, 2))

		self.sprites.empty()
		for m in self.movers:
			if m.rect.left < cameraMax[0] and m.rect.right > cameraMin[0] and m.rect.top < cameraMax[1] and m.rect.bottom > cameraMin[1]:
				self.sprites.add(m)
			else:
				m.update(self.delta)

		for o in self.obstacles:
			if o.minpt[0] < cameraMax[0] and o.maxpt[0] > cameraMin[0] and o.minpt[1] < cameraMax[1] and o.maxpt[1] > cameraMin[1]:
				#o.draw(self.background)
				pygame.draw.lines(self.background, o.color, True, o.points, o.linewidth)
		#pygame.display.flip()
